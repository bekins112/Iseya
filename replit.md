...
- **Transaction History**: Employers see subscription transaction history on Dashboard; Applicants see verification transaction history. API: `GET /api/my-transactions` returns user's transactions. Amount stored in kobo, displayed as Naira (÷100). Dashboard always shows transaction section (with empty state when no transactions).
