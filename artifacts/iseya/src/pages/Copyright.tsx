...
import { ArrowLeft, Copyright as CopyrightIcon, Shield, FileText, AlertCircle, Globe, Scale, BookOpen , Linkedin } from "lucide-react" ;
...
import { copyrightDefaults } from "@/lib/page-content/copyright";

export default function CopyrightPage() {
  usePageTitle("Copyright");
  const c = usePageContent("page_copyright", copyrightDefaults);
  const container = {
    hidden: { opacity: 0 },
    show: {
      opacity: 1,
      ...
      <div className="pt-24 pb-16 px-4 md:px-8 max-w-4xl mx-auto">
        ...
        >
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="text-4xl md:text-5xl font-display font-bold mb-4" data-testid="text-copyright-title">{c.header.title}</h1>
        ...
        >
          <Card className="border-primary/20 bg-primary/5">
            <CardContent className="p-6">
              <p className="text-sm text-muted-foreground">
                {c.contact.note} <Link href="/contact" className="text-primary hover:underline">{c.contact.linkLabel}</Link>.
              </p>
            </CardContent>
          </Card>
        </motion.div>
