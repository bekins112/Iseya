...
import { Users, Briefcase, FileText, Building2, UserCheck, Shield } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const statCards = [
    ...
    { title: "Employers", value: stats?.totalEmployers || 0, icon: Building2, color: "text-purple-600" },
    { title: "Applicants", value: stats?.totalApplicants || 0, icon: UserCheck, color: "text-green-600" },
    { title: "Job Postings", value: stats?.totalJobs || 0, icon: Briefcase, color: "text-amber-600" },
    { title: "Applications", value: stats?.totalApplications || 0, icon: FileText, color: "text-rose-600" },
  ];
        ...
        {adminLinks.map((link) => (
          link.permission !== false && (
            <Link key={link.href} href={link.href}>
              <Card className="hover-elevate cursor-pointer h-full" data-testid={`link-${link.href.split('/').pop()}`}>
